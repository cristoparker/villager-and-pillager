...
/**
 * Shoots an authentic arrow at a hostile monster.
 * @param {Entity} villager 
 * @param {Entity} monster 
 * @param {boolean} isCrossbow 
 * @returns {boolean}
 */
export function shootArrowAtEntity(villager, monster, isCrossbow = false) {
    ...
    // 3. Calculate projectile spawn position (at eye/shoulder height)
    ...
    // 4. Calculate ballistic trajectory towards monster torso
    const targetPos = {
        x: mLoc.x,
        y: mLoc.y + 0.9,
        z: mLoc.z
    ...
    try {
        ...
        console.warn(`[Fletcher] Error spawning arrow: ${e}`);
    }
